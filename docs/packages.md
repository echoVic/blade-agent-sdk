...
| `@blade-ai/agent-sdk/errors` | Browser-safe / Server / CLI | 公共错误层级和稳定错误码 |
...
当前 root session 不会自动注册 Node-local builtin tools。`/local` 的 package-owned baseline 默认返回 `Read`，传入 memory manager 时追加 `MemoryRead` / `MemoryWrite`；返回的预构建 `Tool[]` 可直接作为 `createSession({ tools })` 的显式输入。Edit/Write/Bash/Task 等本地工具仍待迁移。
...
- 写 browser-safe 类型共享代码：用 `@blade-ai/agent-sdk/core`、`@blade-ai/agent-sdk/errors` 和 `@blade-ai/agent-sdk/tools`
