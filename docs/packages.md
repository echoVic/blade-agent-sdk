...
- streaming loop 的 producer/consumer 事件桥、no-tool turn 决策、turn-limit 决策、工具执行计划和工具中断行为可通过 `@blade-ai/agent/loop` 的 `AsyncEventQueue` / `decideNoToolTurn()` / `decideTurnLimit()` / `planToolExecution()` / `resolveToolInterruptBehavior()` / `createInterruptAwareAbortSignal()` 复用
