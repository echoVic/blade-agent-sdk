...
import { AsyncEventQueue, decideNoToolTurn, decideTurnLimit, planToolExecution } from '@blade-ai/agent/loop';
...
- streaming loop 的 producer/consumer 事件桥、no-tool turn 决策、turn-limit 决策和工具执行计划可通过 `@blade-ai/agent/loop` 的 `AsyncEventQueue` / `decideNoToolTurn()` / `decideTurnLimit()` / `planToolExecution()` 复用
