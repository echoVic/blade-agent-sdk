...
7. [x] `registry.ts` 单容器，删 `ToolCatalog`；exposure 收敛为唯一 owner。
8. [x] 清死代码（getMetadata/version/category/tag/stats/双暴露）。
9. [x] 删 `createBuiltinToolGroups`，内置工具改静态数组 + `group` 字段。
10. [x] 根入口只导 `defineTool` + 必要类型。
