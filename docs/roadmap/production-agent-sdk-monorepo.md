...
- Twenty-third adapter increment complete: `@blade-ai/agent-sdk/session` now has a package-local public source entry that exports local session-first contracts (`SessionOptions`, `ISession`, resume/fork/prompt options, user message content, MCP/session status, token budget, hooks, and logger shapes) while wrapping the existing root session runtime. Package declaration emit still comes from the legacy root declaration pass and must be migrated in a later build-system slice.
