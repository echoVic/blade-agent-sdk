...
| Workspace package tests | `pnpm run test:packages` | 直接在 `packages/ai`、`packages/agent`、`packages/agent-sdk` 包目录运行 package-local tests，确保 runtime-independent core 和 provider-layer 测试进入生产 gate |
