...
| Workspace package tests | `pnpm run test:packages` | 直接在 `packages/ai`、`packages/agent`、`packages/agent-sdk` 包目录运行 package-local tests；`@blade-ai/agent-sdk` 不允许 no-tests 通过，确保 session-first facade、browser stub 和后续 package-local runtime slices 都有包内测试落点 |
