...
| Workspace package tests | `pnpm run test:packages` | 直接在 `packages/ai`、`packages/agent`、`packages/agent-sdk` 包目录运行 package-local tests；三包都不允许 no-tests 通过，确保 provider adapter、runtime-independent kernel、session-first facade、browser stub、`allowedTools: []` tool filtering 和后续 package-local runtime slices 都有包内测试落点 |
