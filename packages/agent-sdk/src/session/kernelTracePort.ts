...
export interface PackageLocalKernelTracePortOptions {
  recorder: TraceRecorder;
  maxContextTokens?: number;
}

export function createPackageLocalKernelTracePort(
  options: PackageLocalKernelTracePortOptions,
): AgentTracePort {
    ...
    record(event) {
      switch (event.type) {
        ...
        case 'tool_permission_updates':
          ...
          break;
        case 'usage':
          options.recorder.recordUsage(toTokenUsage(event.usage, options.maxContextTokens ?? 0));
          break;
        case 'turn_end':
          options.recorder.addEvent(
            'turn_end',
