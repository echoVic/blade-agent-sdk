...
export interface PackageLocalStreamingToolExecutorLogger {
  warn(message: string): void;
  error(message: string, error?: unknown): void;
}

const NOOP_STREAMING_TOOL_EXECUTOR_LOGGER: PackageLocalStreamingToolExecutorLogger = {
  warn: () => undefined,
  error: () => undefined,
};
...
interface PackageLocalChatToolDefinition {
  name: string;
  description: string;
...
async function* streamPackageLocalChatResponse(
...
export class PackageLocalStreamingToolExecutor {
  ...
  constructor(
    ...
    this.logger = logger ?? NOOP_STREAMING_TOOL_EXECUTOR_LOGGER;
  }
  ...
  private async emitToolExecutionUpdate(
    ...
    switch (update.type) {
      case 'tool_ready':
        await executionConfig.onToolReady?.(update.toolCall);
        return;
      case 'tool_result':
        await executionConfig.onAfterToolExec?.({
          toolCall: update.outcome.toolCall,
          result: update.outcome.result,
          toolUseUuid: update.outcome.toolUseUuid,
        });
        return;
      case 'tool_completed':
        await executionConfig.onToolComplete?.(
  ...
  private async collectWithFallback(
    ...
    const functionToolCalls = (chatResponse?.toolCalls ?? []).filter(
      (toolCall): toolCall is AgentFunctionToolCall => toolCall.type === 'function',
    );
    ...
    const executeOne = (index: number, toolCall: AgentFunctionToolCall) =>
  ...
  private async executeToolCall(input: {
    ...
    try {
      const outcome = await runPackageLocalToolCall({
        toolCall: input.toolCall,
...
export {
  PackageLocalStreamingToolExecutor as StreamingToolExecutor,
  type PackageLocalStreamingToolExecutorConfig as StreamingToolExecutorConfig,
};
