...
describe('agent-sdk local memory tools', () => {
  it('registers package-local memory tools only when a manager is provided', async () => {
    ...
    await expect(getBuiltinTools()).resolves.toMatchObject([{ name: 'Edit' }, { name: 'Read' }, { name: 'Write' }, { name: 'Grep' }, { name: 'Glob' }, { name: 'NotebookEdit' }, { name: 'AskUserQuestion' }, { name: 'EnterPlanMode' }, { name: 'ExitPlanMode' }, { name: 'TodoWrite' }]);
