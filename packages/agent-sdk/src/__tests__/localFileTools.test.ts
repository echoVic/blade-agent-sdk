...
describe('agent-sdk local file tools', () => {
  ...
  it('includes the full package-local builtin tools provider', async () => {
    ...
    expect(names).toEqual(['AskUserQuestion', 'Bash', 'DiscoverTools', 'Edit', 'EnterPlanMode', 'ExitPlanMode', 'Glob', 'Grep', 'KillShell', 'NotebookEdit', 'Read', 'Skill', 'TodoWrite', 'WebFetch', 'WebSearch', 'Write']);
  });
});
