...
import { FileFilter } from '../filePatterns.js';
...
export function createGlobTool() {
  return createTool({
    ...
    // Zod Schema 定义
    schema: z.object({
      pattern: z.string().min(1).describe('Glob pattern string (supports *, ?, ** wildcards)'),
      path: z.string().optional().describe('Search path (optional, defaults to cwd)'),
      max_results: z.number().int('Must be an integer').min(1, 'Must be greater than 0').max(1000, 'At most 1000 results can be returned').default(100).describe('Maximum number of results'),
    ...
    async execute(params, context: ExecutionContext): Promise<ToolResult> {
      ...
      try {
        if (!(context.contextSnapshot as { context?: { capabilities?: { filesystem?: unknown } } })?.context?.capabilities?.filesystem) {
          return {
            success: false,
