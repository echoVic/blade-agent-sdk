...
import { ToolUseId } from './branded.js';
import { HookEvent } from '../types/constants.js';
import type { PermissionResult } from './permissionTypes.js';
import type { UserMessageContent } from './agentTypes.js';
import type { RuntimeHookRegistration } from './RuntimePatch.js';
import { cloneContentPart } from './messageUtils.js';
import { buildHookInput, isJsonObject, isRecord } from './SessionRuntimeUtils.js';
import type { HookTraceCollector } from './observabilityTypes.js';
import { HookManager } from './HookManager.js';
import { HookBus } from './HookBus.js';
import type { HookCallback } from './HookBus.js';
import type { HookInput as SessionHookInput } from '../session/types.js';
import type { PreToolUseRuntimeResult, PostToolUseRuntimeResult } from './hookTypes.js';

export type { PreToolUseRuntimeResult, PostToolUseRuntimeResult };
...
export class HookRuntime {
  ...
  async applyPreToolUse(
    ...
    if (this.bus.has(HookEvent.PreToolUse)) {
      ...
      for (const output of outputs) {
        ...
        if (output.modifiedInput !== undefined && isRecord(output.modifiedInput)) {
          nextInput = { ...nextInput, ...output.modifiedInput };
        }

        if ((output as unknown as Record<string, unknown>).needsConfirmation) {
          return {
            toolUseId,
            updatedInput: nextInput,
    ...
    // Run project-configured hooks (HookManager)
    const projectDir = this.options.resolveProjectDir();
    if (projectDir) {
      const managerResult = await this.hookManager.executePreToolUseHooks(
        {
          toolName,
          toolInput: nextInput,
          toolUseId,
  ...
  async applyPostToolUse(
    ...
    // Run callback-based hooks first (event bus)
    const callbackResult = await this.applyPostToolCallbacks(
    ...
    nextResult = callbackResult.result;

    if (callbackResult.action === 'abort') {
    ...
    const projectDir = this.options.resolveProjectDir();
    if (projectDir) {
      const managerResult = await this.hookManager.executePostToolUseHooks(
        {
          toolName,
          toolInput: input,
          toolResponse: nextResult,
          toolUseId: toolUseId_,
        },
        projectDir,
        this.options.sessionId,
    ...
    return { toolUseId: toolUseId_, result: nextResult };
  }
  ...
  async applyPostToolUseFailure(
    ...
    const projectDir = this.options.resolveProjectDir();
    if (projectDir) {
      const errorMessage = result.success ? undefined : result.error?.message || `Tool "${toolName}" failed`;
    ...
    const managerResult = await this.hookManager.executePermissionRequestHooks(
      {
        toolName,
        toolInput: nextInput,
        toolUseId: ToolUseId(`permission_${toolName}_${Date.now()}`),
      },
      projectDir,
      this.options.sessionId,
      this.options.permissionMode,
    ...
    if (this.bus.has(HookEvent.UserPromptSubmit)) {
      ...
      for (const output of outputs) {
        ...
        if (output.modifiedInput != null) {
          ...
          } else if (typeof (output.modifiedInput as Record<string, unknown>).userPrompt === 'string') {
            nextMessage = this.replaceTextContent(
              nextMessage,
              (output.modifiedInput as Record<string, unknown>).userPrompt as string,
            );
          }
        }
      }
  ...
  async runTaskCompleted(
    ...
    const result = await this.hookManager.executeTaskCompletedHooks(
      {
        taskSummary: payload.resultSummary,
      },
      projectDir,
      this.options.sessionId,
      this.options.permissionMode,
    ...
    if (!result.allowCompletion) {
      throw new Error(result.warning || result.blockReason || 'Task completed aborted by hook manager');
    }
  }
  ...
  private async dispatchObserved(event: HookEvent, input: SessionHookInput) {
    const spanId = this.traceCollector?.recordHookStart(event, input);
    try {
      const outputs = await this.bus.dispatch(
        event as unknown as Parameters<typeof this.bus.dispatch>[0],
        input as unknown as Parameters<typeof this.bus.dispatch>[1],
      );
      if (spanId) {
