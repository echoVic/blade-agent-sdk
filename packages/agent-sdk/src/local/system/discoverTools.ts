...
This tool searches deferred/discoverable tools, returns the best matches, and activates them for subsequent turns in the current session.`,
  ...
  async execute(params, context): Promise<ToolResult> {
    const searchCatalog = context.toolCatalog as { search(query: string): Array<{ name: string; description: { short: string }; exposure: { mode: string } }> } | undefined;
    const searchSource = searchCatalog ?? context.toolRegistry as { search(query: string): Array<{ name: string; description: { short: string }; exposure: { mode: string } }> } | undefined;
    if (!searchSource) {
      return {
        success: false,
