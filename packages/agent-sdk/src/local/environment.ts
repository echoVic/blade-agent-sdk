...
/**
 * Structured environment information returned by {@link getEnvironmentInfo}.
 */
...
/**
 * Gather system environment information for the current process.
 */
...
/**
 * Format environment information as a human-readable context block suitable for
...
function findProjectRoot(startDir: string): string {
  ...
  while (currentDir !== path.dirname(currentDir)) {
    if (pathExists(path.join(currentDir, '.git'))) return currentDir;
    if (pathExists(path.join(currentDir, 'package.json'))) return currentDir;
