...
import type { AtMention, LineRange } from './promptProcessors.js';
 ...
 * ```
 */
function parseLineRange(path: string): LineRange | undefined {
 ...
 * ```
 */
export function extract(input: string): AtMention[] {
  ...
  while (true) {
    ...
    // 解析行号后缀
    const lineRange = parseLineRange(path);
    if (lineRange) {
      // 移除行号后缀，保留纯路径
      path = path.replace(LINE_RANGE_PATTERN, '');
    }

    // 检测是否为 glob 模式
    const isGlob = GLOB_PATTERN.test(path);
 ...
 * ```
 */
export function removeAtMentions(input: string): string {
  PATTERN.lastIndex = 0;
  return input.replace(PATTERN, '');
}

export const AtMentionParser = { extract, hasAtMentions, isValidPath, removeAtMentions };
