 ...
 */
export class ContextFilterProcessor {
  ...
  static createPresets() {
    return {
      // 轻量级过滤器 - 适合快速响应
      lightweight: new ContextFilterProcessor({
        maxTokens: 1000,
        maxMessages: 10,
      ...
      // 标准过滤器 - 平衡性能和功能
      standard: new ContextFilterProcessor({
        maxTokens: 4000,
        maxMessages: 30,
      ...
      // 完整过滤器 - 包含所有上下文
      comprehensive: new ContextFilterProcessor({
        maxTokens: 8000,
        maxMessages: 100,
        timeWindow: 24 * 60 * 60 * 1000, // 24小时
        includeTools: true,
      ...
      // 调试过滤器 - 专注于错误和工具调用
      debug: new ContextFilterProcessor({
        maxTokens: 2000,
        maxMessages: 20,
