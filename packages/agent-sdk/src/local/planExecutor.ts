...
import { createPlanModeReminder } from './prompts.js';
import { PermissionMode } from '../types/common.js';
import type { AgentEvent } from './agentEvent.js';
import type { ChatContext, UserMessageContent } from './agentTypes.js';
import type { LoopOptions, LoopResult } from './agentLoopTypes.js';
...
export class PlanExecutor {
  ...
  async buildPlanSystemPrompt(context?: ChatContext): Promise<string> {
    const { prompt } = await buildSystemPrompt({
      projectPath: (context?.snapshot as any)?.cwd,
