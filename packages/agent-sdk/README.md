...
The package-owned session runtime 不会自动注册 Node-local builtin tools. Root sessions register explicit `ToolDefinition` values, prebuilt `Tool` values, and configured MCP tools. The current local baseline includes a package-owned `Read` tool and adds `MemoryRead` / `MemoryWrite` when a memory manager is provided; `getBuiltinTools()` can be passed directly to `createSession({ tools })`. Edit/Write/Bash/Task remain separate local-runtime migrations.
