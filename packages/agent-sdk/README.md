...
The package-owned session runtime 不会自动注册 Node-local builtin tools. Root sessions register explicit `ToolDefinition` values, prebuilt `Tool` values, and configured MCP tools. The current local baseline adds `MemoryRead` / `MemoryWrite` only when a memory manager is provided, and `getBuiltinTools()` can be passed directly to `createSession({ tools })`. The broader Read/Edit/Bash/Task suite remains a separate local-runtime migration.
