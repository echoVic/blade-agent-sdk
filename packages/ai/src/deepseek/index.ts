...
import type { Message, ProviderOptions, UsageInfo } from '../chat/index.js';
...
export interface DeepSeekModelDefaultsConfig {
  provider: string;
  model: string;
  baseUrl?: string;
  maxContextTokens?: number;
...
export function withDeepSeekDefaults<T extends DeepSeekModelDefaultsConfig>(modelConfig: T): T {
  ...
  const strictTools = getDeepSeekProviderOptions(modelConfig.providerOptions)?.strictTools ?? false;
  return {
    ...modelConfig,
...
function getDeepSeekProviderOptions(
  providerOptions: DeepSeekModelDefaultsConfig['providerOptions'],
): DeepSeekProviderOptions | undefined {
