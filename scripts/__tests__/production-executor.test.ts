...
type HarnessSession = {
  ...
  suspendForHandoff: ReturnType<typeof vi.fn>;
...
function harness() {
  ...
  let requestCount = 0;
  const handles: HarnessSession[] = [];
  function handle(): HarnessSession {
    const session = {
      sessionId: 'session-1',
  ...
  const createSession = vi.fn(async (_options: RecordData) => {
    trace.push('create');
    return handle();
  });
  const resumeSession = vi.fn(async (_options: RecordData) => {
    trace.push('resume');
    return handle();
  });
  const record = {
  ...
  const store = {
    ...
    putSession: vi.fn(async (value: typeof record) => {
      trace.push('session-record');
    ...
    enqueueSession: vi.fn(
      async (_tenantId: string, _sessionId: string, options: { metadata: RecordData }) => {
        trace.push('enqueue');
  ...
  const cancellations = new Map<
    string,
    { status: string; cleanup?: string; cleanupDetail?: string }
  >();
  const state = {
    permission: { requestId: 'request-1', status: 'pending' },
    update: vi.fn(async (_sessionId: string, patch: RecordData) => {
      trace.push('submission-record');
      return patch;
    }),
    ...
    recordCancellationCleanup: vi.fn(
      async (_sessionId: string, requestId: string, outcome: RecordData) => {
        const cancellation = cancellations.get(requestId);
        if (cancellation) cancellation.cleanup = outcome.succeeded ? 'stopped' : 'failed';
      },
    ),
    getCancellation: vi.fn(async (_sessionId: string, requestId: string) =>
      cancellations.get(requestId),
    ),
    ...
    getPendingSubmission: vi.fn(async (sessionId: string) => {
      ...
      return record
        ? { ...record, value: { ...(record.value as RecordData), input: record.input } }
        : null;
    }),
    ...
    isCancelled: vi.fn(async (_sessionId: string, requestId: string) =>
      cancellations.has(requestId),
    ),
  ...
  const Constructor = exampleClass<new (...args: unknown[]) => Executor>(
    'QueuedSessionExecutor.mjs',
    'QueuedSessionExecutor',
    {
      createSession,
      resumeSession,
...
describe('production QueuedSessionExecutor', () => {
  ...
  it('persists SDK acceptance and releases its fence before making the request claimable', async () => {
    const test = harness();
    let detach!: () => void;
    test.control.detachGate = new Promise<void>((resolve) => {
      detach = resolve;
    });
  ...
  it('accepts the next round with a new API fence and retains the workspace checkpoint', async () => {
    ...
    test.store.route = {
      state: 'idle',
      metadata: { ...queued(), workspaceCheckpoint: { checkpointId: 'checkpoint-1' } },
    };
    const second = await test.executor.submit(
    ...
    expect(test.store.route?.metadata.workspaceCheckpoint).toEqual({
      checkpointId: 'checkpoint-1',
    });
    expect(test.store.route?.metadata.bladeQueuedRequest).toMatchObject({
      requestId: 'request-2',
      input: 'Follow up',
    });
    expect(test.handles.every((session) => session.suspendForHandoff.mock.calls.length === 1)).toBe(
      true,
    );
  });
  ...
  it('serializes simultaneous submissions so only one starts while the route is active', async () => {
    const test = harness();
    let accept!: () => void;
    test.control.sendGate = new Promise<void>((resolve) => {
      accept = resolve;
    });
    const first = test.executor.submit(context, input);
    const second = test.executor.submit(
      { ...context, commandId: 'command-2' },
      { ...input, input: 'Conflicting request' },
    );
  ...
  it('records the acceptance before anything else can fail after the journal write', async () => {
    ...
    expect(test.trace.indexOf('submission-accepted')).toBeLessThan(
      test.trace.indexOf('submission-record'),
    );
    expect(test.state.recordSubmissionAccepted).toHaveBeenCalledWith({
      sessionId: 'session-1',
      requestId: 'request-1',
  ...
  it.each([
    ...
    {
      label: 'different execution options',
      input: 'Inspect the repository',
      commandId: 'command-1',
      maxTurns: 8,
    },
  ])('rejects a new submission for %s while recovery is pending', async ({
    input: nextInput,
    commandId,
    maxTurns,
  }) => {
  ...
  it('detaches failed submissions and never queues a request whose handoff failed', async () => {
    ...
    await expect(test.executor.submit(context, input)).rejects.toThrow(
      'The fence could not be released',
    );
    expect(test.store.enqueueSession).not.toHaveBeenCalled();
  });
  ...
  it('reads the tenant transcript projection and rejects cross-tenant submissions', async () => {
    ...
    await expect(
      test.executor.submit(
        { ...context, principal: { tenantId: 'other-tenant', subject: 'other-user' } },
        input,
      ),
  ...
  it('accepts only once-scoped decisions for the current pending request', async () => {
    ...
    await expect(
      test.executor.resolvePermission(context, { ...decision, scope: 'session' }),
    ).rejects.toMatchObject({ protocolCode: 'INVALID_COMMAND' });
    expect(test.state.resolvePermission).not.toHaveBeenCalled();
    await test.executor.resolvePermission(context, {
      ...decision,
      scope: 'once',
      reason: 'Reviewed',
    });
    expect(test.state.resolvePermission).toHaveBeenCalledWith(
      'session-1',
      'request-1',
      'permission-1',
      {
    ...
    await expect(
      test.executor.resolvePermission(context, { ...decision, approved: false }),
  ...
  it.each([
    ...
    'cancelled',
    'idle',
  ] as const)('rejects an approval after %s', async (reason) => {
    const test = harness();
    ...
    await expect(
      test.executor.resolvePermission(context, {
        sessionId: 'session-1',
        permissionRequestId: 'permission-1',
        approved: true,
      }),
  ...
  it('keeps duplicate cancellation commands pending until the Worker acknowledges the shared intent', async () => {
    ...
    let acknowledged = 0;
    const cancel = test.executor.abort(context, input).then(() => {
      acknowledged += 1;
    });
    const duplicate = test.executor
      .abort({ ...context, commandId: 'command-2' }, input)
      .then(() => {
        acknowledged += 1;
      });
  ...
  it.each([
    ...
    'completed',
    'failed',
  ])('acknowledges cancellation when execution reaches %s', async (terminal) => {
    vi.useFakeTimers();
    const test = harness();
    test.store.route = { state: 'running', metadata: queued() };
    let acknowledged = false;
    const cancelled = test.executor.abort(context, input).then(() => {
      acknowledged = true;
    });
    await vi.advanceTimersByTimeAsync(50);
    ...
    expect(acknowledged).toBe(false);
    const stoppedCancellation = test.cancellations.get('request-1');
    expect(stoppedCancellation).toBeDefined();
    if (!stoppedCancellation) throw new Error('cancellation was not recorded');
    stoppedCancellation.cleanup = 'stopped';
    await vi.advanceTimersByTimeAsync(50);
  ...
  it('refuses to confirm cancellation when the execution environment was not stopped', async () => {
    ...
    const failedCancellation = test.cancellations.get('request-1');
    expect(failedCancellation).toBeDefined();
    if (!failedCancellation) throw new Error('cancellation was not recorded');
    failedCancellation.cleanup = 'failed';
    failedCancellation.cleanupDetail = 'docker rm failed';
    await vi.advanceTimersByTimeAsync(50);
...
describe('production RepositoryState permission binding', () => {
  function stateHarness() {
    ...
    const Constructor = exampleClass<
      new (
        options: RecordData,
      ) => {
    ...
    >('RepositoryState.mjs', 'RepositoryState', {
      pg: {
        Pool: class {
          query = query;
        },
    ...
    const request = {
      toolName: 'RepoWrite',
      input: { path: 'report.md', content: 'Reviewed content' },
    };
    const record = {
      requestId: 'request-1',
      request,
  ...
  it('reuses the existing decision only for the same tool and equivalent input', async () => {
    ...
    expect(test.query.mock.calls[0]?.[1]).toEqual([
      ...
      JSON.stringify({
        toolName: 'RepoWrite',
        input: { content: 'Reviewed content', path: 'report.md' },
      }),
    ]);
  });
  ...
  it.each([
    ...
    'input',
    'request',
  ] as const)('rejects reused permission IDs when the %s changes', async (changed) => {
    const test = stateHarness();
    const request = {
      toolName: changed === 'tool' ? 'RepoDelete' : 'RepoWrite',
      input: {
        path: 'report.md',
        content: changed === 'input' ? 'Unreviewed replacement' : 'Reviewed content',
      },
    };
    await expect(
      test.state.requestPermission({
        ...test.options,
  ...
  it('rejects decisions after the pending permission has already been consumed or retired', async () => {
    ...
    await expect(
      test.state.resolvePermission('session-1', 'request-1', 'permission-1', {
        approved: true,
        scope: 'once',
      }),
    ).rejects.toMatchObject({ protocolCode: 'PERMISSION_NOT_FOUND' });
