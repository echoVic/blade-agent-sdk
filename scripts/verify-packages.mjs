...
const packageSpecs = [
  ...
  },
  {
    ...
    forbiddenFileContents: [
      ...
      ...toPackedForbiddenFileRules(agentSdkRootSubagentCompatibilityBoundaryRules),
...
const packedReadonlyBehavior = agentSdkTools.createToolBehavior(agentSdkTools.ToolKind.ReadOnly);
if (
  packedReadonlyBehavior.kind !== 'readonly'
  || !packedReadonlyBehavior.isReadOnly
  || !packedReadonlyBehavior.isConcurrencySafe
) {
  throw new Error('@blade-ai/agent-sdk/tools behavior helper returned an unexpected result');
}
console.log('packed sdk behavior readonly true true');
...
function verifyConsumerTypes(consumerDir) {
  const localDeclaration = readFileSync(
...
async function verifyConsumerBrowserBundle(consumerDir) {
  ...
  writeFileSync(
    ...
    [
      ...
      "try { createReadTool(); } catch (error) { console.log((error as Error).message); }",
      "try { FileAccessTracker.getInstance(); } catch (error) { console.log((error as Error).message); }",
    ].join('\n'),
  );
  ...
  if (!browserRunOutput.includes('browser-safe sdk behavior readonly true true')) {
    throw new Error('Browser bundle does not include the browser-safe SDK tool behavior runtime smoke');
  }
  ...
  if (!browserRunOutput.includes('server-only for createReadTool')) {
    throw new Error('Browser bundle does not include the createReadTool server-only stub message');
  }
  if (!browserRunOutput.includes('server-only for FileAccessTracker.getInstance')) {
    throw new Error('Browser bundle does not include the FileAccessTracker server-only stub message');
  }
...
async function verifyAgentBrowserBundle(consumerDir) {
  ...
  if (!browserRunOutput.includes('finish stop serial cancel tool_start true true')) {
