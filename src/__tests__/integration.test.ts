...
const PROVIDER_TYPE = (process.env.INTEGRATION_PROVIDER_TYPE ||
...
describeIntegration('1. prompt() 一次性调用', () => {
  it(
    '应返回非空文本回复',
    async () => {
      const res: PromptResult = await prompt('请用一句话回答：1+1等于几？', baseOptions());
...
describeIntegration('4. 自定义工具调用', () => {
    ...
    async () => {
      ...
      const session = await createSession(
        baseOptions({
          tools: [weatherTool],
          systemPrompt:
            'You have access to a get_weather tool. When asked about weather, use it. Respond concisely.',
        }),
      );
      activeSessions.push(session);
      ...
      const toolUseEvents: SessionStreamEvent[] = [];
      const toolResultEvents: SessionStreamEvent[] = [];
      let result = '';
  ...
  it(
    ...
    async () => {
      ...
      console.log(
        '[multi-tool] tool calls:',
        res.toolCalls.map((tc) => `${tc.name}(${JSON.stringify(tc.input)})`),
      );
...
describeIntegration('6.1 持久化恢复与高级分叉', () => {
  it(
    'resumeSession 应恢复持久化上下文并继续流式对话',
    async () => {
  ...
  it(
    'forkSession 应允许父会话和分支会话沿不同上下文继续演化',
    async () => {
...
describeIntegration('6.2 Hooks / Permissions / MCP 真实链路', () => {
  it(
    ...
    async () => {
      ...
      await session.send(
        '调用 echo_hook 一次，value 传 raw-input。最后只回复 EXACT:post-hooked-output。',
      );
  ...
  it(
    ...
    async () => {
      ...
      expect(toolUseEvents.length).toBeGreaterThanOrEqual(1);
      expect(toolResultEvents.length).toBeGreaterThanOrEqual(1);
      expect(JSON.stringify(toolResultEvents[0])).toContain(
        'permission denied for integration test',
      );
  ...
  it(
    '应能通过 in-process MCP server 暴露工具并由模型调用',
    async () => {
      ...
      expect(mcpTools.some((toolInfo) => toolInfo.name.includes('lookup_release_train'))).toBe(
        true,
      );
...
describeIntegration('6.3 子代理 / 后台代理真实链路', () => {
  it(
    ...
    async () => {
      ...
      expect(
        toolUseEvents.some((event) => event.type === 'tool_use' && event.name === 'Task'),
      ).toBe(true);
  ...
  it(
    '应能启动后台子代理并通过 TaskOutput 获取最终结果',
    async () => {
      const session = await createSession(
        baseOptions({
...
describeIntegration('6.4 多模态 / MCP 恢复真实链路', () => {
  it(
    ...
    async () => {
      ...
      const errorMessages = streamed.events
        .filter(
          (event): event is Extract<SessionStreamEvent, { type: 'error' }> =>
            event.type === 'error',
        )
  ...
  it(
    'mcpDisconnect 和 mcpReconnect 后应恢复工具调用能力',
    async () => {
      const handle = await createSdkMcpServer({
      ...
      expect(initialTools.some((toolInfo) => toolInfo.name.includes('lookup_recovery_code'))).toBe(
        true,
      );
      ...
      expect(
        disconnectedTools.some((toolInfo) => toolInfo.name.includes('lookup_recovery_code')),
      ).toBe(false);

      await session.mcpReconnect('recovery');
      const reconnectedTools = await session.mcpListTools();
      expect(
        reconnectedTools.some((toolInfo) => toolInfo.name.includes('lookup_recovery_code')),
      ).toBe(true);
...
describeIntegration('8. 结构化输出', () => {
  it(
    'outputFormat 应返回符合 schema 的 JSON（或被正确传递到 provider）',
    async () => {
      const res = await prompt(
      ...
      if (jsonMatch) {
        try {
          ...
          console.log(
            '[structured-output] JSON parsing failed, provider may not support structured output',
          );
        }
      } else {
        // 无 JSON 匹配，但结果存在说明 prompt 本身成功
        console.log(
          '[structured-output] no JSON found in response, provider may not support structured output',
        );
      }
  ...
  it(
    '严格 schema 模式一：应返回完全符合 checklist schema 的 JSON',
    async () => {
      const res = await prompt(
  ...
  it(
    '严格 schema 模式二：应返回完全符合 routing schema 的 JSON',
    async () => {
      const res = await prompt(
...
describeIntegration('10. 并发 Session', () => {
  it(
    '多个独立 session 可并发运行',
    async () => {
      const questions = ['1+1等于？只回复数字。', '2+2等于？只回复数字。', '3+3等于？只回复数字。'];
