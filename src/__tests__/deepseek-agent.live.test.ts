...
describeDeepSeek('1. Thinking output 在 Agent 中的透传', () => {
  it(
    'stream({ includeThinking: true }) 应返回 thinking delta',
    async () => {
...
describeDeepSeek('2. Thinking + Tool Use 组合场景', () => {
  ...
  it(
    ...
    async () => {
      ...
      await session.send(
        '请使用 calculate 工具计算 (17 * 23) + (45 / 9) 的结果。你必须调用工具，不要自己计算。',
      );
      const { content, thinking, events } = await drainStream(session, { includeThinking: true });
    ...
    async () => {
      const dbTool = defineTool({
        ...
        async execute(params) {
          ...
          return JSON.stringify({ table: params.table, records, count: records.length });
        },
      });
      ...
      console.log(
        '[multi-turn-tool] tool calls:',
        res.toolCalls.map((tc) => tc.name),
      );
    ...
    async () => {
      const failTool = defineTool({
        ...
        async execute(params) {
          ...
          throw new Error(
            `API Error: endpoint "${params.endpoint}" returned 503 Service Unavailable`,
          );
        },
      });
  ...
  it(
    ...
    async () => {
      ...
      if (u1?.type === 'usage' && u2?.type === 'usage') {
        console.log(
          '[token-growth] round1:',
          u1.usage.inputTokens,
          '| round2:',
          u2.usage.inputTokens,
        );
  ...
  it(
    ...
    async () => {
      const createTaskTool = defineTool({
        ...
        async execute(params) {
          return JSON.stringify({ id: 'TASK-001', created: true, ...params });
        },
      });
      ...
      console.log(
        '[complex-schema] tool calls:',
        JSON.stringify(
          res.toolCalls.map((tc) => tc.input),
          null,
          2,
        ),
      );
  ...
  it(
    ...
    async () => {
      const statusTool = defineTool({
        ...
        async execute(params) {
          return JSON.stringify({ updated: true, ...params });
        },
      });
...
describeDeepSeek('5. 结构化输出兼容性', () => {
  it(
    'outputFormat JSON 应在 thinking 模型下正常工作',
    async () => {
      const res = await prompt(
...
describeDeepSeek('6. 指令遵从与角色扮演', () => {
  it(
    '应严格遵从 systemPrompt 的输出格式约束',
    async () => {
      const res = await prompt(
        '解释什么是 REST API',
        baseOptions({
          systemPrompt:
            '你是一个技术文档助手。所有回复必须遵循以下格式：\n## 定义\n[一句话定义]\n## 要点\n- 要点1\n- 要点2\n- 要点3\n\n不要添加任何其他内容。',
        }),
      );
  ...
  it(
    ...
    async () => {
      const session = await createSession(
        baseOptions({
          systemPrompt:
            '你是一个 JSON-only 回复机器人。所有回复必须是合法 JSON 对象，格式为 {"answer": "...", "confidence": 0.0-1.0}。不要输出任何非 JSON 内容。',
        }),
      );
      activeSessions.push(session);
...
describeDeepSeek('7. 并发与边界情况', () => {
  it(
    ...
    async () => {
      ...
      await Promise.all(tasks.map((task, i) => sessions[i].send(task)));

      const results = await Promise.all(sessions.map((s) => drainStream(s)));

      console.log(
        '[concurrent]',
        results.map((r) => r.content.slice(0, 50)),
      );
