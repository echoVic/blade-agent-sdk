...
const allowedTransitions = {
  ...
  running: ['running', 'waiting_approval', 'suspended', 'idle', 'completed', 'failed'],
  waiting_approval: ['waiting_approval', 'running', 'suspended', 'failed'],
  suspended: ['suspended', 'queued', 'provisioning', 'idle', 'completed', 'failed'],
  idle: ['idle', 'queued', 'completed', 'failed'],
  completed: ['completed'],
  failed: ['failed'],
} as const;
  ...
  it('defines every transition for every state', () => {
    for (const from of RUNTIME_SESSION_STATES) {
      for (const to of RUNTIME_SESSION_STATES) {
        ...
        expect(canTransitionRuntimeSession(from, to), `${from} -> ${to}`).toBe(allowed);
        if (allowed) {
          expect(() => assertRuntimeSessionTransition(from, to)).not.toThrow();
        } else {
          expect(() => assertRuntimeSessionTransition(from, to)).toThrow(/cannot transition/);
        }
      }
    }
  });
